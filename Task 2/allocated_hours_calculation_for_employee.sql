DROP VIEW IF EXISTS allocated_hours_calculation_for_employee;

CREATE VIEW allocated_hours_calculation_for_employee AS
SELECT 
cv.course_code, 
ci.instance_id, 
cv.hp,   
...
WHERE e.emp_id = 3 AND ci.year = EXTRACT(YEAR FROM CURRENT_DATE);

SELECT * FROM allocated_hours_calculation_for_employee;